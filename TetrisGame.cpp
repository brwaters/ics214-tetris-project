...
TetrisGame::TetrisGame(sf::RenderWindow *window, sf::Sprite *blockSprite,
                       Point gameboardOffset, Point nextShapeOffset) {
  pWindow = window;
...
void TetrisGame::drawBlock(int x, int y, TetColor color, Point origin) {
  ...
  pBlockSprite->setTextureRect(
      sf::IntRect(intColor * 32, 0, BLOCK_WIDTH, BLOCK_HEIGHT));
  pBlockSprite->setPosition(origin.getX() + x * BLOCK_HEIGHT,
  ...
  for (int x = 0; x < board.MAX_X; x++) {
    for (int y = 0; y < board.MAX_Y; y++) {
      if (board.getContent(x, y) != Gameboard::EMPTY_BLOCK) {
        drawBlock(x, y, static_cast<TetColor>(board.getContent(x, y)),
                  gameboardOffset);
      }
    }
  }
...
bool TetrisGame::isShapeWithinBorders(const GridTetromino &shape) {
  for (auto loc : shape.getBlockLocsMappedToGrid()) {
    if (loc.getX() < board.MAX_X && loc.getX() >= 0 &&
        loc.getY() < board.MAX_Y && loc.getY() >= -4) {
    } else {
      return false;
    }
  }
...
void TetrisGame::determineSecsPerTick() { secsPerTick = MAX_SECS_PER_TICK; }
